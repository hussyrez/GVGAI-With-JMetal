...
public class NPointCrossover<T> implements CrossoverOperator<Solution<T>> {
  ...
  private List<Solution<T>> doCrossover(List<Solution<T>> s) {
    ...
    if (mom.getNumberOfVariables() < crossovers) { // Min: was > before, but this does not match the prints.
      //System.out.println("mom.getNumberOfVariables()=" + mom.getNumberOfVariables());
