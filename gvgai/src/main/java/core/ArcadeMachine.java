...
import static core.competition.GVGReplayer.actionFile;
...
public class ArcadeMachine
{
                                ...
                                String agentName, String[] actionFiles, int exercise)
    {
        ...
        String vict = "", sc = "", stdDevVic = "", stdDevSc = "";
        for (int i = 0; i < toPlay.no_players; i++) {
            vict += victories[i].mean();
            sc += scores[i].mean();
            stdDevVic += victories[i].sd();
            stdDevSc += scores[i].sd();
        ...
        // Output results for exercise 1 to file
        try{
            // Getting game name without path and file extension
            String game_name = new File(game_file).getName();
            game_name = game_name.substring(0, game_name.lastIndexOf('.'));

            // Storing result in results/exercise#/game_name/
            String resultFileName = "results/exercise" + exercise + "/";
            resultFileName += game_name + "/";
            // filename: game_name-cntrl-controller.txt
            resultFileName += game_name + "-cntrl-" + agentName.substring(25, agentName.lastIndexOf(".")); // add controller name
            resultFileName += ".txt";
            new File(resultFileName).getParentFile().mkdirs();
            PrintWriter writer = new PrintWriter(resultFileName);
            writer.write("Results in game " + game_file + "\n" +
                    "Victory stats (mean, stdDev): " + vict + " | " + stdDevVic +
                            "\nScore stats (mean, stdDev): " + sc + " | " + stdDevSc);
            writer.close();
        } catch (Exception e){
            e.printStackTrace();
        }
