...
public class Test
{
    ...
    public static void main(String[] args)
    {
        ...
        String multiStepController = "controllers.singlePlayer.multisteplookahead.Agent";
        String random = "controllers.singlePlayer.random.Agent";
        ...
        // 0 = aliens, 9 = boulderdash, 11 = butterflies, 16 = chase
        int gameIdx = 0;
        ...
        //ArcadeMachine.runOneGame(game, level1, visuals, suntzuController, recordActionsFile, seed, 0);
        //Game and level to play

        // Plays a game 5 times for the first 3 levels
//        for (int j = 0; j < 3; j++) {
//            level1 = gamesPath + games[gameIdx] + "_lvl" + j +".txt";
//            for (int i = 0; i < 5; i++) {
//                ArcadeMachine.runOneGame(game, level1, false, suntzuController, recordActionsFile, seed, 0);
//            }
//            System.out.println(">>>>>>>> LEVEL " + j + " END <<<<<<<<<");
//        }

        int[] games_index = {0, 9, 11, 16};
        for(int g : games_index ){
            for (int j = 4; j < 5; j++) {
                level1 = gamesPath + games[g] + "_lvl" + j +".txt";
                for (int i = 0; i < 5; i++) {
                    ArcadeMachine.runOneGame(game, level1, false, suntzuController, recordActionsFile, seed, 0);
                }
                System.out.println(">>>>>>>> LEVEL " + j + " END <<<<<<<<<");
            }
            System.out.println(">>>>>>>> GAME " + g  + " END <<<<<<<<<");
        }

        //ArcadeMachine.runGames(game, new String[]{level1}, 20, multiStepController, null, 1);
